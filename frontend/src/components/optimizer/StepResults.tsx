...
export function StepResults({ results, selectedStrategy, loanIds, monthlyExtra, lumpSums, annualGrowthPct }: Props) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="py-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200">
          ...
          <div className="grid grid-cols-2 gap-3 px-6">
            <div className="bg-white/70 rounded-lg p-3 text-center border border-gray-200">
              <p className="text-xs text-gray-500 mb-1">{t("optimizer.results.withoutPlan")}</p>
              <p className="text-lg font-bold text-gray-700">{fmtC(Number(results.baseline_total_interest))}</p>
              <p className="text-xs text-gray-400">{formatMonths(results.baseline_total_months)}</p>
            </div>
                ...
                return (
                  <div key={loan.loan_id}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="font-medium text-gray-700">{loan.bank_name}</span>
      ...
      <div>
        <h3 className="font-semibold text-gray-900 mb-3">{t("optimizer.results.comparison")}</h3>
            ...
            >
              ...
              {s.loan_results.length > 0 && (
                <div className="mt-3 pt-2 border-t border-gray-200">
                  <p className="text-xs text-gray-400 mb-1">{t("optimizer.results.perLoanBreakdown")}</p>
                  {s.loan_results.map((lr) => (
                    <div key={lr.loan_id} className="flex items-center justify-between text-xs text-gray-600 py-0.5">
                      <span>{lr.bank_name} <span className="text-gray-400">({lr.loan_type})</span></span>
